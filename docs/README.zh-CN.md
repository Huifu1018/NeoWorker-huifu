...
当前源码在侧边栏新增**论文动态**，通过官方公开接口获取 arXiv 论文、Hugging Face 每日精选和近期活跃的 GitHub 项目。支持最多五个关注方向、来源筛选、已获取内容搜索，以及重启后保留的本地收藏。来源使用 arXiv、Hugging Face 和 GitHub 官方品牌标识，来源面板沿用自动化页面的统一浅蓝底，论文卡片和标签保持中性色，主要操作和选中状态使用蓝色。宽屏每行展示三张卡片，窄窗口自动调整列数，支持深浅主题。
