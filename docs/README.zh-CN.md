...
当前源码在侧边栏新增**资讯动态**，通过官方公开接口获取 arXiv 论文、Hugging Face 每日精选和近期活跃的 GitHub 项目。三个来源分别设置最多五个关注词和时间范围：arXiv 可指定学科分类，GitHub 可指定编程语言和最低 Star 数，Hugging Face 可仅展示每日精选中匹配关注词的内容。旧版共用设置会自动迁移，保存时只刷新当前来源，保留其他来源缓存和收藏。支持来源筛选、已获取内容搜索，以及重启后保留的本地收藏。来源使用 arXiv、Hugging Face 和 GitHub 官方品牌标识，来源面板沿用自动化页面的统一浅蓝底，论文卡片和标签保持中性色，主要操作和选中状态使用蓝色。宽屏每行展示三张卡片，窄窗口自动调整列数，支持深浅主题。

卡片采用上方横向封面、下方标题与摘要的版式。封面随内容获取：arXiv 原文图片或 PDF 首页缩略图、Hugging Face 论文缩略图、GitHub 项目预览图；无法获取时使用带当前标题的文字封面。进入可视区域才加载，并使用持久化本地缓存，不会每次刷新都重新生成 AI 配图。

卡片提供原始来源和论文 PDF 链接。**阅读、翻译、深入研究**会准备新的任务草稿，由你确认发送后使用当前配置的模型执行。论文翻译要求保留图片、表格和公式并输出完整 PDF；项目翻译针对 README。 当前源码会直接从原 PDF 区域渲染高清图，裁剪图目标宽度至少 2400 像素，不再依赖放大页面预览截图。导出时检查大图在最终排版尺寸下的有效清晰度，翻译交付也会拦截低清配图；目标为 300 DPI，原图本身不清晰时仍需说明限制。
