...
The ACP adapter is opt-in through the task's external runtime configuration; ordinary NeoWorker tasks continue to use the native execution path. Hermes owns tool execution for ACP tasks, while NeoWorker mediates permission requests and records runtime events. ACP permission callbacks do not cover every operation and are not a sandbox. A pinned-source adapter remains a viable implementation path. The adapter exposes safe session-boundary pause/resume primitives for integration tests and host callers; the desktop task pause controller still needs to connect these primitives before Hermes tasks can advertise UI pause/resume as complete.
