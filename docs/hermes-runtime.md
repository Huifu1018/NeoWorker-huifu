...
`HermesToolHostMcpServer` exposes only the task's filtered catalog on an ephemeral `127.0.0.1` port with a random bearer token. Every tool call enters the existing executor Tool Host, and the persisted result is returned to Hermes as a bounded MCP tool result. Session-scoped correlation prevents reused MCP request IDs from aliasing calls after reconnect. Request timeout and runtime shutdown propagate cancellation into the host operation. Loopback addresses are added to the child process's `NO_PROXY`/`no_proxy` so local tool traffic cannot accidentally go through an inherited HTTP proxy.
