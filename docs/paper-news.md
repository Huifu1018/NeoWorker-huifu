...
The feed uses three cards per row on wide panels, two on medium panels, and one on narrow panels. Each source uses its official brand mark in the compact source panel and paper cards. The unmodified SVGs are bundled locally, with suitable light/dark variants. Neutral cards and topic chips keep the feed visually consistent; blue highlights primary actions and selection. See [brand asset provenance](../src/renderer/assets/paper-news/README.md). Ranking details are available under **About ranking and sources**.
