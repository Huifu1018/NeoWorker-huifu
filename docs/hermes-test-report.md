...
- 全量 CI（2026-09-10，提交 `77ccc59`）为 853 个测试文件：795 通过、54 失败、4 跳过；失败集中在既有 mailbox/managed/memory/renderer/Office 等环境或基线测试，本轮 Hermes、Shell 和 Windows runner 专项未出现新增回归。`npm run lint` 为 0 错误，剩余为既有警告。
