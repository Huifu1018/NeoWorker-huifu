...
| 持久 Shell 长输出边界 | 通过集成回归 | 持久会话内部缓冲限制为 1,000,000 字符，结果限制为 100KB 并保留尾部；分块累积避免 Windows 大量小 chunk 触发 O(n²) 字符串复制；长输出后下一条命令可继续执行 |
| Windows Hermes 启动路径 | 通过单元验证 | `hermes-host-launcher.test.ts` 覆盖显式 Python、Hermes shebang 和带引号 PATH 目录，避免含空格的虚拟环境路径回退到错误解释器 |
| Windows runner 执行链 | 通过 CI | CI run `34497421168` 的 `windows-agent-runtime` 在 `windows-latest` 上完成 Electron 构建与 Hermes/Tool Host/Shell 专项测试；15 个测试文件、190 个测试通过，1 个平台限定测试跳过 |
| 长任务与故障注入 | 通过专项验证 | `hermes-fault-injection.test.ts` 验证 32 项只读任务受并发上限约束、失败读取不污染缓存、副作用调用即使请求并发也保持串行；checkpoint 只读取最新任务事件并复用单次工具进度快照 |
...
| Hermes 任务级运行时路由 | 已接入并通过专项测试 | 新建任务固定使用内置 Hermes ACP Harness；前端不再暴露 `Auto / Hermes / Native` 选择；旧任务保留历史 runtime，Hermes 启动失败不降级 |
| Hermes 运行时状态可见性 | 已接入并通过专项测试 | Hermes active、Native active 和 forced failure 都投影为明确的时间线状态；用户看到实际 runtime 和错误码，不显示已废弃的 preference 选择 |
...
| macOS ARM64 安装包 | 通过 | CI run [`34503347147`](https://github.com/Huifu1018/NeoWorker-huifu/actions/runs/34503347147) 完成构建、DMG 挂载、版本、签名、Numbat、Runtime 内容、启动 smoke 和 artifact 上传 |
...
本轮已完成最终跨平台打包验证。macOS ARM64 DMG 在 run `34503347147` 中构建、安装后 smoke 通过并上传为 artifact `NeoWorker-0.1.8-3-macos-arm64`；Windows x64 安装器在 run `34501098120` 中构建、安装后 smoke 通过并上传为 artifact `NeoWorker-0.1.8-3-windows-x64`。Windows smoke 失败过一次，原因是校验脚本在 Windows 上没有把 ASAR 反斜杠路径转换为统一格式，已在提交 `9140b64` 修复并复验通过。

已下载到本机的测试包：

- macOS ARM64 DMG：`/Users/wangchao/Downloads/NeoWorker-0.1.8-3-macos-arm64/NeoWorker-0.1.8-3-arm64.dmg`
- Windows x64 安装器：`/Users/wangchao/Downloads/NeoWorker-0.1.8-3-windows-x64/NeoWorker-0.1.8-3-windows-x64-setup.exe`
...
- 打包后的 NeoWorker 自带版本固定的 Hermes ACP host executable，不需要用户额外安装 Hermes Agent。源码开发模式可以通过 `NEOWORKER_HERMES_PYTHON` 指定本地 Python 解释器。
- 新建 NeoWorker 任务固定使用内置 Hermes ACP；简单问答也不再因为提示词短而切换 Native。历史 Native 任务仍按原路径恢复，避免旧会话被静默迁移。
...
1. 使用已上传的 macOS 和 Windows artifact 进行实际安装测试。
2. 后续改动继续沿用当前 Windows/macOS smoke 门禁，避免未验证的桌面包进入交付。
