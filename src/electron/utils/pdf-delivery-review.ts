...
export class PdfDeliveryReviewer {
  ...
  async review(workspace: string, candidates: string[], sourcePaths: string[] = [], signal?: AbortSignal): Promise<PdfDeliveryReview[]> {
...
export function buildPdfRepairInstruction(reviews: PdfDeliveryReview[]): string {
  return [
    "PDF LAYOUT REPAIR: one targeted correction pass for the existing requested deliverable.",
