...
Editorial cards use a landscape cover above the title and summary. Covers come from the content itself: arXiv figures or a PDF first-page thumbnail, Hugging Face paper thumbnails, and GitHub repository previews. Missing images use a title-based text cover. Visible cards load covers lazily and reuse a persistent local cache; refreshing the feed does not generate new AI illustrations.

Each card links to its source and, for papers, the PDF. **Read**, **Translate**, and **Research** prepare a new task draft for your configured model. Paper translation requests a complete PDF with figures, tables, and correctly typeset equations; repository translation targets the README. Review and send the draft to start the task. Source builds now render figure crops directly from the PDF at export resolution (targeting at least 2400 pixels wide), rather than enlarging overview screenshots. PDF export checks the effective printed resolution of large raster images; translation delivery also rejects low-resolution figures. The target is 300 effective DPI; low-resolution originals still require an explicit limitation.
